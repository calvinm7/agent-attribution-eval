...
**Reproduction** ([secs. 1](REPORT.md#1-reproduction-vs-published-numbers) & [2](REPORT.md#2-data-splits-and-feature-verification))
...
**Metric framing** ([sec. 3](REPORT.md#3-metric-choice-on-a-single-confusion-matrix))
...
**Site scaling** ([sec. 5](REPORT.md#5-cross-site-attribution-and-training-site-scaling))
...
**Split record** ([sec. 2](REPORT.md#2-data-splits-and-feature-verification))
...
**Replication drift** ([sec. 7](REPORT.md#7-extension-attempt-harness-decay-across-a-four-month-gap))
...
Start with the writeup:

- **[REPORT.md](REPORT.md)** — full results: reproduction deltas, open-set, site scaling, extension attempt
...
The paper's traces are not in this repo (the release has no license). `fetch_data.py` downloads them into `data/traces/`. Features and models are rebuilt locally.

**Extension** ([REPORT sec. 7](REPORT.md#7-extension-attempt-harness-decay-across-a-four-month-gap))

| file | role |
|---|---|
| `ext_proxy.py` | usage-recording proxy for re-collection |
| `ext_collect.py` | trace collection for newer Claude versions |
| `data/webshop_questions.json` | WebShop questions used for collection |
| `data/new_traces/` | traces I collected |
| `data/ext_collection_log.jsonl` | episode-level collection log |
| `data/ext_usage.jsonl` | token usage recorded by the proxy |
